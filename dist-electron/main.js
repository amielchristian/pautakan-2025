        ...
        db.get("SELECT COUNT(*) as count FROM college", (err2, row) => {
          ...
          if (row.count === 0) {
            ...
            colleges.forEach((college) => {
              stmt.run(college.name, college.shortHand, college.imagePath, 0, (err3) => {
                completed++;
                if (err3) {
                  console.error(`Error inserting college ${college.shortHand}:`, err3);
                }
                if (completed === total) {
                  stmt.finalize();
    ...
    return new Promise((resolve, reject) => {
      db.run("UPDATE college SET score = score + ? WHERE shortHand = ?", [increment, shortHand], function(err) {
        if (err) {
          ...
          db.get("SELECT * FROM college WHERE shortHand = ?", [shortHand], (err2, row) => {
            if (err2) {
              console.error(`Error getting updated college ${shortHand}:`, err2);
              reject(err2);
            } else {
              BrowserWindow.getAllWindows().forEach((window) => {
...
function createWindow() {
  try {
    ...
    mainWindow.webContents.on("did-finish-load", () => {
      mainWindow == null ? void 0 : mainWindow.webContents.send("main-process-message", "Window in fullscreen mode");
